        ...
        child: BlocBuilder<EnergyBloc, EnergyState>(
          builder: (context, state) {
            ...
            return ListView(
              ...
              children: [
                AppSectionHeader(title: 'Manual Reading Entry', subtitle: 'Record energy meter readings'),
                if (_metersLoading)
                  const AppSkeletonCard()
                else if (_meters.isEmpty)
                  const AppEmptyState(icon: Icons.speed_rounded, title: 'No meters configured', subtitle: 'Add one in Settings first')
                else
                  Form(
                    key: _formKey,
                    child: Column(
                      ...
                      children: [
                        AppCard(
                          child: Column(
                            ...
                            children: [
                              const Text('Meter Selection', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                              ...
                              DropdownButtonFormField<String>(
                                ...
                                items: _meters.map((m) => DropdownMenuItem(value: m.name, child: Text(m.name))).toList(),
                                onChanged: (v) {
                                  if (v != null) {
                              ...
                              if (_fetchingPrevious) const Padding(
                                padding: EdgeInsets.only(top: 8),
                                child: Row(children: [SizedBox(width: 14, height: 14, child: CircularProgressIndicator(strokeWidth: 2)), SizedBox(width: 8), Text('Fetching previous reading...', style: TextStyle(fontSize: 12, color: AppColors.textSecondary))]),
                              ),
                            ],
                          ),
                        ),
                        ...
                        AppCard(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text('Energy Readings', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                              const SizedBox(height: 12),
                              Row(children: [
                                Expanded(child: AppTextField(controller: _currentKwhCtrl, label: 'Current kWh', prefixIcon: Icons.bolt, keyboardType: TextInputType.numberWithOptions(decimal: true), validator: (v) => v == null || v.trim().isEmpty ? 'Required' : null)),
                                const SizedBox(width: 12),
                                Expanded(child: AppTextField(controller: _previousKwhCtrl, label: 'Previous kWh', hint: 'Auto-fetched', prefixIcon: Icons.history, keyboardType: TextInputType.numberWithOptions(decimal: true), validator: (v) => v == null || v.trim().isEmpty ? 'Required' : null)),
                              ]),
                              const SizedBox(height: 12),
                              Row(children: [
                                Expanded(child: AppTextField(controller: _currentKvahCtrl, label: 'Current kVAh', prefixIcon: Icons.electrical_services, keyboardType: TextInputType.numberWithOptions(decimal: true), validator: (v) => v == null || v.trim().isEmpty ? 'Required' : null)),
                                const SizedBox(width: 12),
                                Expanded(child: AppTextField(controller: _previousKvahCtrl, label: 'Previous kVAh', hint: 'Auto-fetched', prefixIcon: Icons.history, keyboardType: TextInputType.numberWithOptions(decimal: true), validator: (v) => v == null || v.trim().isEmpty ? 'Required' : null)),
                              ]),
                            ],
                          ),
                        ),
                        ...
                        AppCard(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text('Power Quality', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                              const SizedBox(height: 12),
                              Row(children: [
                                Expanded(child: AppTextField(controller: _rkvarhLagCtrl, label: 'rkVARh (Lag)', prefixIcon: Icons.warning_outlined, keyboardType: TextInputType.numberWithOptions(decimal: true))),
                                const SizedBox(width: 12),
                                Expanded(child: AppTextField(controller: _rkvarhLeadCtrl, label: 'rkVARh (Lead)', prefixIcon: Icons.check_circle_outline, keyboardType: TextInputType.numberWithOptions(decimal: true))),
                              ]),
                              const SizedBox(height: 12),
                              AppTextField(controller: _mdRecordedCtrl, label: 'MD Recorded (kW)', prefixIcon: Icons.trending_up, keyboardType: TextInputType.numberWithOptions(decimal: true), validator: (v) => v == null || v.trim().isEmpty ? 'Required' : null),
                            ],
                          ),
                        ),
