  ...
  Widget build(BuildContext context) {
    ...
    for (final log in logs) {
      if (log.loggedAt.year == now.year && log.loggedAt.month == now.month) {
        ...
        dailyMap.update(day, (v) => v + log.kwh * AppConstants.multiplyingFactor, ifAbsent: () => log.kwh * AppConstants.multiplyingFactor);
      }
    }

    if (dailyMap.isEmpty) {
      return const SizedBox(height: 200, child: Center(child: Text('No monthly data', style: TextStyle(color: AppColors.textSecondary))));
    }
    ...
    return SizedBox(
      ...
      child: LineChart(LineChartData(
        minX: 0, maxX: (dayNumbers.length - 1).toDouble().clamp(0, double.infinity), minY: 0, maxY: chartMaxY,
        ...
        titlesData: FlTitlesData(
          ...
          bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 28,
            ...
            getTitlesWidget: (value, meta) {
              ...
              if (idx < 0 || idx >= dayNumbers.length) return const SizedBox.shrink();
              return Padding(padding: const EdgeInsets.only(top: 4),
                child: Text('${dayNumbers[idx]}', style: TextStyle(fontSize: 9, color: AppColors.textSecondary)));
            })),
        ...
        lineBarsData: [
          LineChartBarData(spots: spots, isCurved: true, curveSmoothness: 0.3, color: AppColors.primary,
        ...
        lineTouchData: LineTouchData(touchTooltipData: LineTouchTooltipData(
          getTooltipItems: (touchedSpots) => touchedSpots.map((spot) => LineTooltipItem(
            ...
            TextStyle(color: AppColors.primary, fontWeight: FontWeight.bold, fontSize: 12),
          )).toList(),
        )),
      )),
    );
